...
class GridTests(unittest.TestCase):
    ...
    def test_grid_Initialization(self):
        ...
        expected_grid = np.zeros(expected_size, dtype=np.float32)
    ...
    def test_color_Initialization(self):
        ...
        for i in range(grid.grid.shape[0]):
            for j in range(grid.grid.shape[1]):
                self.assertTrue(grid.grid[i,j]==expected_color)
    ...
    def test_color_of_Coordinate(self):
        ...
        self.assertTrue(grid.color_of(coord)==expected_color)

    #def test_draw_Positive(self):
    #    grid = Grid(self.grid_size, self.unit_size)
    #    expected_color = np.array(grid.BODY_COLOR, dtype=np.float32)
    #    coord = [3,2]
    ...
    #    expected_colors = np.array([grid.HEAD_COLOR, grid.BODY_COLOR, grid.BODY_COLOR], dtype=np.float32)
